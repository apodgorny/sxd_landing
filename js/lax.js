  ...
  const laxInstance = (() => {
    ...
    function flattenStyles(styles) {
      ...
      Object.keys(styles).forEach((key) => {
        ...
        const unit = pxUnits.includes(key)
          ? "px"
          : degUnits.includes(key)
          ? "deg"
          : "";
    ...
    class LaxElement {
      ...
      calculateTransforms = () => {
        ...
        for (let driverName in this.transformsData) {
          ...
          for (let key in styleBindings) {
            let [arr1 = [-1e9, 1e9], arr2 = [-1e9, 1e9], options = {}] =
              styleBindings[key];

            const bounds = this.domElement.getBoundingClientRect();
            const parsedArr1 = getArrayValues(arr1, windowWidth).map((i) =>
              parseValue(i, bounds, this.groupIndex)
            );
            const parsedArr2 = getArrayValues(arr2, windowWidth).map((i) =>
              parseValue(i, bounds, this.groupIndex)
            );
    ...
    class Lax {
      ...
      onWindowResize = () => {
        const changed =
          document.body.clientWidth !== this.windowWidth ||
      ...
      onAnimationFrame = (e) => {
        ...
        if (this.debug) {
          this.debugData.frameLengths.push(
            Date.now() - this.debugData.frameStart
          );
        }

        if (this.frame % 60 === 0 && this.debug) {
          const averageFrameTime = Math.ceil(
            this.debugData.frameLengths.reduce((a, b) => a + b, 0) / 60
          );
      ...
      addDriver = (name, getValueFn, options = {}) => {
        this.drivers.push(new LaxDriver(name, getValueFn, options));
      };
      ...
      findAndAddElements = () => {
        ...
        elements.forEach((domElement) => {
          ...
          this.elements.push(
            new LaxElement(".lax", this, domElement, transforms, 0, {})
          );
        });
      };
      ...
      addElements = (selector, transforms, options) => {
        ...
        domElements.forEach((domElement, i) => {
          this.elements.push(
            new LaxElement(selector, this, domElement, transforms, i, options)
          );
        });
      };
      ...
      addElement = (domElement, transforms, options) => {
        this.elements.push(
          new LaxElement("", this, domElement, transforms, 0, options)
        );
      };

      removeElement = (domElement) => {
        this.elements = this.elements.filter(
          (element) => element.domElement !== domElement
        );
      };
    }
